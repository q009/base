...
namespace fx
{
    ...
    static emitter *foo = NULL;
    static int foomillis;

    void update()
    {
        if(foo && lastmillis - foomillis < 10000) foo->prolong();
    ...
    ICOMMAND(0, testfx, "si", (char *name, int *sameinstance),
    {
        ...
        createfx(fxindex, from, to, 1, 1, bvec(255, 255, 255), NULL, *sameinstance ? &foo : NULL);
        foomillis = lastmillis;
    });
}
